import React from 'react';
import {CloseIcon, DotIcon} from '@eruditorgroup/profi-icons';
import {Story, Meta} from '@storybook/react';

import GeoTag, {GeoTagProps} from './index';
import Space from '../Space';

export default {
  title: 'GeoTag',
  component: GeoTag,
} as Meta;

const base64 =
  '';

const Template: Story<GeoTagProps> = (args) => {
  return (
    <>
      <Space bg="light" px={20} py={20}>
        <h2>Plain design</h2>
        <GeoTag {...args} trailing={<CloseIcon />}>
          Some tg
        </GeoTag>
        <GeoTag {...args} icon={<DotIcon color="red" />}>
          Some tg
        </GeoTag>
        <GeoTag {...args} icon={<DotIcon color="blue" />} trailing={<CloseIcon />}>
          Some tg
        </GeoTag>
        <GeoTag
          {...args}
          icon={{icon: base64, format: 'base64Svg'}}
          trailing={<CloseIcon />}
        >
          Some tg
        </GeoTag>
      </Space>
      <Space px={20} py={20}>
        <h2>Light design</h2>
        <GeoTag {...args} design="light" trailing={<CloseIcon />}>
          Some tg
        </GeoTag>
        <GeoTag {...args} design="light" icon={<DotIcon color="red" />}>
          Some tg
        </GeoTag>
        <GeoTag
          {...args}
          design="light"
          icon={<DotIcon color="blue" />}
          trailing={<CloseIcon />}
        >
          Some tg
        </GeoTag>
        <GeoTag
          {...args}
          design="light"
          icon={{icon: base64, format: 'base64Svg'}}
          trailing={<CloseIcon />}
        >
          Some tg
        </GeoTag>
      </Space>
    </>
  );
};

export const Default = Template.bind({});
Default.args = {};
